...
#include <libopencm3/stm32/timer.h> // 包含延时函数的头文件
#include <libopencm3/stm32/gpio.h>
 ...
 **************************************************/
void indcpa_keypair(unsigned char *pk, unsigned char *sk, unsigned char *rr)
{
  polyvec skpv;
  poly pkp;
                                          ...
                                          const unsigned char *sk)
{
  ...
  {
    test_v.coeffs[num] = 624;
    // test_mp.coeffs[num] = 0;
    // test_mp.coeffs[num] = 767;
    // test_mp.coeffs[num] = 1534;
    // test_mp.coeffs[num] = -1534;
    test_mp.coeffs[num] = -767;
    //  if (num < 50){
    //  test_mp.coeffs[num] = -1534;
  ...
  // gpio_set(GPIOA, GPIO7);
  // delay_us(10);
  poly_sub(&test_mp, &test_v, &test_mp);
  // gpio_clear(GPIOA, GPIO7);
